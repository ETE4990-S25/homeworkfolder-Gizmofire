...
if __name__ == "__main__":
    ...
    NROWS = 3
    NFRAMES_START = 0
    NFRAMES_STOP = 20 # 3600 can take over an hour to complete....
    NFRAMES_TOTAL = 36 # 3600 can take over an hour to complete....

    # Useful quantities
    NTHREADS = 3#data.index.levels[0].size
    ...
    for frame in tqdm(range(NFRAMES_START, NFRAMES_STOP), leave=False, desc="frame"):  # animation frames
        ...
        for ax, t_lim in zip(axs, t_lims):  # loop over each axis
            ...
            ax.set_yticklabels(
                np.arange(0, NTHREADS+1, 1, dtype=int))
